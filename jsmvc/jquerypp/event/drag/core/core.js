define(
    ['jquery', 'jquerypp/lang/vector/vector', 'jquerypp/event/livehack/livehack', 'jquerypp/event/reverse/reverse'],
    function( $ ) {
        ...
        var stopEvents = ["touchend", "mouseup"];
        ...
        $.extend($.Drag.prototype, {
            setup: function( options, ev ) {
                ...
                for (var i in moveEvents) {
                    $(document).bind(moveEvents[i], mousemove);
                }
                for (var j in stopEvents) {
                    $(document).bind(stopEvents[j], mouseup);
                }
                if(supportTouch) {
                    // On touch devices we want to disable scrolling
                    for (var k in moveEvents) {
                        $(document).bind(moveEvents[k], preventTouchScroll);
                    }
                }
            ...
            destroy: function() {
                ...
                for (var i in moveEvents) {
                    $(document).unbind(moveEvents[i], this._mousemove);
                }
                for (var j in stopEvents) {
                    $(document).unbind(stopEvents[j], this._mouseup);
                }
                if(supportTouch) {
                    // Enable scrolling again for touch devices when the drag is done
                    for (var k in moveEvents) {
                        $(document).unbind(moveEvents[k], preventTouchScroll);
                    }
                }
        ...
        event.setupHelper(eventTypes, startEvents, function( e ) {
