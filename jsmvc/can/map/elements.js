define(['can/util/jquery', 'can/map'], function(can, Observe) {

    //!steal-remove-start
    can.dev.warn("can/map/elements is a deprecated plugin, Mathspace is " +
        "providing a shim for the time being.");
    //!steal-remove-end
	...
	can.extend(can.Map.prototype,{
		...
		identity: function() {
            ...
            //!steal-remove-start
            if (!name && this.constructor.model) {
                var keys = Object.keys(this.attr()).toString();
                throw new Error('elements:identity() - Model missing _fullName object with keys: ' + keys);
            }
            //!steal-remove-end
		...
		hookup: function( el ) {
            ...
            //!steal-remove-start
            if (!shortName && this.constructor.model) {
                var keys = Object.keys(this.attr()).toString();
                throw new Error('elements:hookup() - Model missing _shortName for object with keys ' + keys +  ' for element ' + $el.attr('class'));
            }
            //!steal-remove-end
