...
'src test Makefile package.json'.split(' ').forEach(function(filename) {
  recursivelyWatch(filename, run_make_mathspace);
});
...
function run_make_mathspace() {
  if (q) return;
  q = [];
  console.log('[%s]\nmake mathspace', (new Date).toISOString());
  var make_mathspace = child_process.exec('make mathspace', { env: process.env });
  make_mathspace.stdout.pipe(process.stdout, { end: false });
  make_mathspace.stderr.pipe(process.stderr, { end: false });
  make_mathspace.on('exit', function(code) {
